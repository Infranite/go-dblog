...
首个公开 tag 目标是 `v0.1.0`：面向已有数据库日志文件、导出记录或捕获流用户的
离线解析版本。Roadmap 可以在正式 tag 发布前标记该目标为 Ready；首个 tag 发布前，
建议使用检出的分支进行评估。在线 replication reader 已规划，但不属于当前发布线。
...
当前公开目标：`v0.1.0`，离线 parser developer preview 的首个 GitHub Release 和
tag 集合。
