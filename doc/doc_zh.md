...
|[`github.com/Infranite/go-dblog/mysql`](../mysql)|MySQL 族 binlog 和 replication stream：MySQL、MariaDB、MySQL-compatible 方言|已支持|
|[`github.com/Infranite/go-dblog/postgres`](../postgres)|PostgreSQL 族 logical decoding 解析、SQL slot reader 和 replication protocol reader|已支持|
|[`github.com/Infranite/go-dblog/mongo`](../mongo)|MongoDB 族 oplog / change stream JSON 解析和 live change stream reader|已支持|
|[`github.com/Infranite/go-dblog/redis`](../redis)|Redis 族 AOF RESP 解析和 replication stream reader|已支持|
...
当前还没有发布公开 tag。当前公开目标是 `v0.1.0` tag 集合：面向已有数据库日志
文件、导出记录、捕获流或各 backend 已文档化 live source 用户的首个可用版本。
首个 tag 发布前，建议使用检出的分支或 commit 进行评估。已发布能力和待补能力以
[`ROADMAP.md`](../ROADMAP.md#capability-matrix) 中的能力矩阵为准。

|Backend|`v0.1.0` 支持输入|暂不包含|
|---|---|---|
|MySQL|本地 MySQL 族 binlog 文件；在线 replication stream|GTID auto-positioning 和 TLS DSN|
|PostgreSQL|logical decoding 文本记录；基于 `test_decoding` 的 SQL logical slot 轮询和 wire-level replication|`pgoutput` 二进制 relation/tuple 解码|
|MongoDB|按行分隔的 oplog 或 change stream JSON 记录；replica set live collection change stream|JSON 记录或 change stream 之外的原始 oplog tailing|
|Redis|Redis AOF RESP 数组命令；PSYNC replication stream|Cluster/Sentinel 发现和 TLS DSN|
...
详细路线图和能力矩阵见 [`ROADMAP.md`](../ROADMAP.md)。版本范围、已发布能力和 CI
证据只在 roadmap 中维护，避免多处表格漂移。

当前还没有发布公开 tag。当前公开目标：`v0.1.0`，首个可用 developer preview
tag 集合。

## 版本

GitHub Releases 和 git tags 是公开发布记录。Git history 是详细变更记录，因此项目
不单独维护发布说明或 changelog 文件。

- 根模块 tag 使用 `vX.Y.Z`。
- backend module tag 使用 `mysql/vX.Y.Z`、`mongo/vX.Y.Z`、
  `postgres/vX.Y.Z` 和 `redis/vX.Y.Z`。
- `v0.x` 阶段 backend module 版本跟随根模块版本。
...
贡献通过 pull request 处理。本节就是项目级贡献说明；项目不单独维护
`CONTRIBUTING.md`。
