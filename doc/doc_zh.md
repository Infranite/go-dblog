...
首个公开 tag 目标是 `v0.1.0`：面向已有数据库日志文件、导出记录、捕获流或
PostgreSQL `test_decoding` logical slot 用户的解析版本。Roadmap 可以在正式 tag
发布前标记该目标为 Ready；首个 tag 发布前，建议使用检出的分支进行评估。大部分在线
replication reader 仍在后续发布线中规划。
...
当前公开目标：`v0.1.0`，parser developer preview 的首个 GitHub Release 和 tag
集合。
...
发布记录使用 GitHub Releases 和 Git tags 管理。根模块使用 `vX.Y.Z`；backend
module 使用带模块前缀的 tag，例如 `mysql/vX.Y.Z`、`mongo/vX.Y.Z`、
`postgres/vX.Y.Z` 和 `redis/vX.Y.Z`。仓库不维护独立 release 文档或 changelog；
GitHub Releases 是公开 release notes，完整细节以 git history 为准。
