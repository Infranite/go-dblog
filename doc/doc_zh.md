...
当前公开目标是 `v0.1.0`（尚未发布）：面向已有数据库日志文件、导出记录或捕获流
用户的离线解析版本。在线 replication reader 已规划，但不属于当前发布线。
...
当前公开目标：`v0.1.0`（尚未发布），离线 parser developer preview。
...
发布使用 GitHub Releases 和 Git tags 管理。根模块使用 `vX.Y.Z`；backend module
使用带模块前缀的 tag，例如 `mysql/vX.Y.Z`、`mongo/vX.Y.Z`、`postgres/vX.Y.Z`
和 `redis/vX.Y.Z`。仓库不维护独立 release 文档或 changelog；GitHub Releases 是
公开 release notes，完整细节以 git history 为准。
