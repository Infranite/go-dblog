...
下一个公开 tag 目标是 `v0.1.0`：面向已有数据库日志文件、导出记录或捕获流用户的
离线解析版本。在线 replication reader 已规划，但不属于当前发布线。
...
|版本|状态|主题|
|---|---|---|
|`v0.1.0`|待打 tag|离线 parser developer preview|
|`v0.2.0`|计划中|兼容性加固|
...
和 `redis/vX.Y.Z`。Release notes 写在 GitHub Releases 中；完整细节以 commit
history 为准。
