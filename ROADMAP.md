...
|---|---|---|---|---|
| `v0.1.0` | In progress | Offline parser developer preview | Root common API, MySQL binlog file parser, PostgreSQL logical decoding text parser, MongoDB JSON line parser, Redis RESP AOF parser, plugin hooks, filtering, basic flashback helpers. | `make lint`, `make test`, `make test-mysql`, `go test -race`, `go vet`, `govulncheck`; README documents offline scope and module tags. |
